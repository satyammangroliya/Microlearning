...
abstract class AbstractSingleGUI implements SingleGUIInterface
{
    ...
    public function render() : string
    {
        ...
        $tpl = self::plugin()->template("TileSingle/customized_single.html");
	  ...
	  author: minervis(jephte.abijuru@minervis.com)
          ...
          */
	   $tpl_views=self::plugin()->template("Views/views.html");
       $tpl_views->setVariableEscaped("VIEWS_IMAGE_PATH", self::plugin()->directory() . "/templates/images/eye.svg");

       $tpl_views_count=self::plugin()->template("Views/views_count.html");
       $views_count=$this->getViewsCount($this->tile->_getIlObject()->getID());
	   $tpl_views_count->setVariableEscaped("VIEWS_COUNT", $views_count);
	   $tpl_views->setVariable("VIEWS_COUNT",self::output()->getHTML($tpl_views_count));
	   ...
	   if($this->tile->getShowLaptop()===1){
	      ...
	      $tpl_devices->setVariableEscaped("LAPTOP", self::plugin()->directory() ."/templates/images/devices/Laptop_NA.svg");
	   }

	   $tpl->setVariable("DEVICES", self::output()->getHTML($tpl_devices));
            /* End of Views functionalities*/
            ...
            if ($this->tile->getEnableRating() === Tile::SHOW_TRUE
                ...
                if (self::srTile()->ratings(self::dic()->user())->hasLike($this->tile->getObjRefId())) {
                    ...
                    $tpl_rating->setVariableEscaped("RATING_IMAGE_PATH", self::plugin()->directory() . "/templates/images/star.svg");
                }
