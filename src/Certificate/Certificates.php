            ...
            case "crs":
                ...
                if ($this->enabled_core()) {
                    if (ilCourseParticipants::getDateTimeOfPassed($tile->_getIlObject()->getId(), $this->user->getId())) {
                        self::dic()->ctrl()->setParameterByClass(ilObjCourseGUI::class, ilToGoUIHookGUI::GET_PARAM_REF_ID, $tile->getObjRefId());
            ...
            case "sahs":
                if ($this->enabled_core()) {
                    if (self::srTile()->ilias()->learningProgress($this->user)->getStatus($tile->getObjRefId()) === ilLPStatus::LP_STATUS_COMPLETED_NUM) {
                        ...
                        //$this->ctrl->setParameterByClass(ilSAHSPresentationGUI::class, ilToGoUIHookGUI::GET_PARAM_REF_ID, $obj_ref_id);
